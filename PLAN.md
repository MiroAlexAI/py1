...
## Этап 5: Интеграция логики и интерфейса
1.  **Связка UI и Async**: Использование `QThread` или `qasync` для интеграции событийного цикла asyncio с циклом событий PyQt (избежание "замерзания" интерфейса).
2.  **Поиск и Сортировка**: Применение `QSortFilterProxyModel` ко всем табличным представлениям.

## Этап 6: Финализация и Экспорт
1.  **Сохранение**: Логика переноса выбранных (Selected) строк из временной памяти в постоянную БД.
2.  **Экспорт данных**: Реализация выгрузки результатов в форматах Markdown (с использованием таблиц) и JSON.
3.  **Полировка**: Применение современных стилей (CSS/QSS) для обеспечения "Rich Aesthetics".
